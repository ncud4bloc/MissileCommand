...
function update(){
    for (var i = 0; i < bullets.length; i++){
        if (bullets[i].y < myClicks[i][1]){
            ...
            bullets[i].x += deltaAr[i][0];
            bullets[i].y -= deltaAr[i][1];
        }
    }
    return;
}
...
$(function(){
    ...
    $start.on('click',function(){ 
        ...
        for (var i = 0; i < misslesPerIter; i++){
            var missEL = new MissileGen('missEL'+i,5,true);
            missEL.pickTarget();
            missEL.addMissile();
            missEL.drawMissile();
        ...
        /*var missleTarget = Math.floor(Math.random()*9 + 1);
            switch(missleTarget) {
                case 1:
                    missiles[i].tX = cities[0].x;
                    missiles[i].tY = cities[0].y;
                    break;
                case 9:
                    missiles[i].tX = guns[2].x;
                    missiles[i].tY = guns[2].y;
            }*/
